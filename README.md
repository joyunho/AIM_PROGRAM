...
2. 플레이리스트 `AIMDESK Day / Friday / Probe` 가 코박스 Local Playlists 에 자동 설치됩니다.
3. **▶ 오늘 루틴 실행** — 코박스를 띄우고, 오늘 칠 시나리오 전체가 순서대로 나열된 **순서창**이 열립니다.
   순서창은 항상 위에 떠 있고, 판이 끝날 때마다 자동으로 ✓ 체크되며 다음 판에 ▶ 가 붙습니다. ("순서 보기"로 언제든 다시 열 수 있음)
