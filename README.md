...
4. 코박스 ESC 메뉴 상단, 초록색 "플레이" 버튼 왼쪽의 [프리 플레이 | 도전 과제] 토글이 "프리 플레이" 면 타이머도 기록도 없으니 "도전 과제"(CHALLENGE) 로.
...
- **타이머가 안 보이면 코박스가 FREEPLAY 모드입니다.** 딥링크는 코박스의 현재 모드(FREEPLAY/CHALLENGE)를 그대로 따르고 이 모드는 게임이 기억하므로,
  ...
  이미 바뀌었다면 게임에서 ESC → 상단 "플레이" 버튼 왼쪽 토글을 "도전 과제"(CHALLENGE) 로 → 순서창 "다시 보내기". 오른쪽에 "프리플레이 매니저" 창이 떠 있으면 프리플레이 상태입니다. 앱은 보낸 지 100초가 지나도 기록이 없으면 이 경고를 띄웁니다.
